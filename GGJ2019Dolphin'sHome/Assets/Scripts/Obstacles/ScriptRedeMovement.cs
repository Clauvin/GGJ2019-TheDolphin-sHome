...
public class ScriptRedeMovement : MonoBehaviour
{
    ...
    void Update()
    {
        MoveRedeForward(directional_speed);

        UpdateValues();
    }

    public void MoveRedeForward(Vector3 directional_speed)
    {
        Vector3 final_directional_speed = GetFinalDirectionalSpeed(directional_speed);

        float change_x, change_y;
        change_x = final_directional_speed.y;
        change_y = -final_directional_speed.x;

        final_directional_speed.x = change_x;
        final_directional_speed.y = change_y;
    ...
    {
        float final_rotation_speed = (rotation_in_degrees + 90.0f) * ScriptGlobalVariables.game_speed;
